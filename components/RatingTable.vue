<template>
	<div class="content_box">
             ...
             <div class="grid grid-cols-12 gap-4">
                 ...
                 <div v-if="filtersData?.distance" class="col-span-12 sm:col-span-6 lg:col-span-2">
                     <SelectField
                             ...
                             nullable
                             placeholder="Все"
                             :initial-value="initialValues.distance"
                     />
                 </div>
                 <div v-if="filtersData?.stroke_name" class="col-span-12 sm:col-span-6 lg:col-span-2">
                     <SelectField
                             ...
                             nullable
                             placeholder="Все"
                             :initial-value="initialValues.stroke_name"
                     />
                 </div>
                ...
                <CollapsibleContent>
                    <div class="grid grid-cols-12 gap-4 mt-4">
                        ...
                        <div class="col-span-12">
                           <CheckboxField name="one_result" label="Только один результат для спортсмена" label-class="text-white flex items-center" />
                        </div>
                    </div>
                </CollapsibleContent>
            </Collapsible>
        ...
        >
            ...
            <Column field="athlete_name" header="Спортсмен" sortable>
                <template #body="{data}">
                    <NuxtLink :to="{name: 'athlete', params: {id: data.athlete_id}}" class="flex items-center text-main">
                        <Avatar :class="'me-2'" :name="data.athlete_name" :src="data.athlete_avatar"/>
                        <span class="font-bold text-sm">{{ data.athlete_name }}</span>
                    </NuxtLink>
                </template>
            </Column>
...
const initialValues = {
    ...
    search_for: 'athlete_name',
    ranking_place_min: 1,
    course: 'SCM',
    gender: 'M',
    distance: 50,
    stroke_name: 'Вольный стиль'
}
const pagination = ref({
    sort_by: 'ranking_place',
    sort_dir: 'asc',
    page: 1,
    per_page: 20,
})
...
const {data: tableData, refresh: refreshTableData, status: statusTableData} = useAsyncData('ratings', () => getResults())
...
async function getResults(){
    ...
    if(request.one_result){
        request.one_result_by = 'athlete_name'
    }
