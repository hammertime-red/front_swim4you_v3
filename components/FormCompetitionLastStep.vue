<template>
    ...
    <div class="content_box p-4 mb-4">
        ...
        <div class="row g-4">
            <div v-if="STAGE.name && STAGE.id" class="col-md-4 text-sm">
                <p class="text-sub mb-1">Мероприятие</p>
                <router-link :to="`/stage/${STAGE.id}`" class="mb-0 font-bold">
                    {{STAGE.name}}
                </router-link>
            </div>
            <div v-if="STAGE.location_address" class="col-md-4 text-sm">
                <p class="text-sub mb-1">Место проведения</p>
                <p class="mb-0">
                    {{STAGE.location_address}}
                </p>
            </div>
            <div v-if="STAGE.dates" class="col-md-4 text-sm">
...
export default {
    ...
    computed:{
        ...
        athleteIds(){
            return this.EXTRA_ITEMS.map(item => item.athlete_id).filter(item => !!item)
        },
        ...mapGetters({
