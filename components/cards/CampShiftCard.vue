<template>
    <div class="pricetable_box" :class="item.special ? 'special' : ''">
        ...
        <div class="pricetable_box__content">
            <p v-if="status == 'open'" class="font-bold text_20 text-positive">
                Есть места
            </p>
            <p
                v-if="status == 'last_places'"
                class="font-bold text_20 text-accent"
            >
                Осталось {{ item.tickets_count }} мест
            </p>
            <p
                v-if="status == 'end_places'"
                class="font-bold text_20 text-red-500"
            >
                Нет мест
            </p>
            <p v-if="status == 'end_event'" class="font-bold text_20 text-red-500">
                Смена завершена
            </p>
            <p v-if="status == 'closed'" class="font-bold text_20 text-red-500">
                Регистрация закрыта
            </p>
            <p v-if="status == 'end_places'" class="mb-0 text-red-500">
            ...
            <a
                ...
                @click.prevent="globalStore.setOffcanvasData(offcanvasData), globalStore.toggleOffcanvas('camp_shift')"
