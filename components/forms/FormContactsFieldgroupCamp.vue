        ...
        <div class="row gy-3">
            ...
            <div class="col-md-4 mb-3 mb-md-0">
                <div class="form-check">
                    <label class="form-check-label small font-bold" for="auth_accept">
                        Представитель – это я
                    </label>
    ...
    <div class="row g-3">
        <div class="col-lg-6">
            <label class="d-block">
                ...
                <p v-if="show_errors && !agent_name" class="mb-0 small text-danger">Обязательное поле</p>
            </label>
        </div>
        <div class="col-lg-6">
            <label class="d-block">
                ...
                <input 
                    ...
                    required>
                <p v-if="show_errors && !agent_phone" class="mb-0 small text-danger">Обязательное поле</p>
            </label>
        </div>
        <div class="col-lg-6">
            <label class="d-block">
                ...
                <p v-if="show_errors && !agent_email" class="mb-0 small text-danger">Обязательное поле</p>
            </label>
        </div>
        <div class="col-lg-6">
            <label class="d-block">
                <span class="font-bold">Город <span class="text-danger">*</span></span>
                <select  
                    v-model="agent_city"
                    class="form-select"
                    :class="show_errors && !agent_city ? 'invalid' : ''">
                ...
                <p v-if="show_errors && !agent_city" class="mb-0 small text-danger">Обязательное поле</p>
            </label>
        </div>
...
export default {
    ...
    setup(props, { emit }) {
        ...
        const agent_city = computed({  
            get: () => props.agent_city,
            set: (value) => emit('update:agent_city', value),
        });
        const has_errors = computed({  
            get: () => props.has_errors,
            set: (value) => emit('update:has_errors', value),
        });
        return { agent_name, agent_phone, agent_email, agent_city, has_errors, globalStore};
    },
    data(){
        return {
            its_me: false
    ...
    methods:{
        ...
        validateFields(){
            if(!this.agent_name || !this.agent_phone || !this.agent_email || !this.agent_city){
    ...
    emits: ['update:agent_name','update:agent_phone','update:agent_email','update:agent_city','update:has_errors'],
}
</script>
